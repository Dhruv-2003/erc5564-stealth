...
pub fn check_stealth_address(
    ...
    let (shared_secret, _) =
        get_shared_secret(decode_pubkey(&ephemeral_pubkey), decode_priv(&viewing_key));
...
pub fn check_stealth_address_fast(
    ...
    let (shared_secret, view_tag_check) =
        get_shared_secret(decode_pubkey(&ephemeral_pubkey), decode_priv(&viewing_key));
...
pub fn generate_stealth_meta_address() -> (StealthMetaAddress, PrivateKey, PrivateKey) {
    ...
    return (
        encode_stealth_meta_address(pks, pkv),
        encode_priv(s),
        encode_priv(v),
    );
}
...
pub fn get_address_from_pubkey(pubkey: k256::ProjectivePoint) -> Address {
    let result = PublicKey::try_from(pubkey);

    let pub_reformat = match result {
        Ok(val) => val,
        Err(error) => panic!("problem generating PublicKey: {:?}", error),
    };
    let pub_uncompressed = pub_reformat.to_encoded_point(false);

    let mut hasher = Keccak256::new();
    hasher.update(&pub_uncompressed.as_bytes()[1..]);
